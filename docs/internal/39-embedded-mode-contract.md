...
## Core finding
The SDKs are ~70% embed-ready (they already have `chrome`, `features`, `documentMode`, `collab`, `user`, `fileSource`/`onSave`). The **one true gap**: there is **no chrome level that hides the app shell (logo/menubar/About/Help) while keeping the editing toolbar**. `chrome:"full"` renders the SDK's TitleBar+MenuBar (→ the "two shells"); `chrome:"none"` also kills the toolbar. Docs is the offender; **sheets is already ~native** (no logo, no File▸Open, Cmd+S already saves) except a "View on GitHub" Help link + thin menus.
...
1. **The editing-only chrome level** (`mode:'embedded'` / a `chrome:'embedded'` preset): hide TitleBar/MenuBar/logo/About **independently of the toolbar gate** (docs: `DocxEditor.tsx` TitleBar `:9851` + MenuBar `:9865` are welded to `showToolbarEffective` `:2065` — split them). This single change kills the two-shells + logo + menubar + About.
